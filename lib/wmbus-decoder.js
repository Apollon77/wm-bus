...
WMBUS.prototype.decodeLinkLayerMBus = function (linklayer) {

    this.unpack('Cframetype/', linklayer);
    app.log.debug("frametype " + this.frametype);
    if (this.frametype !== 0x68) {
        app.log.debug("ignore frame");
        return 0;
    };

    this.unpack('Clfield/Clfield2/Cstartfield/Ccfield/Cafield/Ccifield/', linklayer.substring(1));
    if (this.lfield !== this.lfield2) {
        app.log.debug("lfield different ... " + this.lfield + " / " + this.lfield2);
        return 0;
    }
    //this.manufacturer = manId2ascii(this.mfield).toUpperCase();
    //this.afield_id = decNo(decodeBCD(8, linklayer.substr(4, 4)));
...
WMBUS.prototype.parseMBus = function (x) {
    var decrypted = this.decrypted;
    this.msg = x;
    this.errormsg = '';
    this.errorcode = cc.ERR_NO_ERROR;
    if (this.decodeLinkLayerMBus(this.msg.substr(0, 12)) != 0) {
        this.linkLayerOk = 1;
        this.decodeApplicationLayer();
        this.updateStates();
    }
